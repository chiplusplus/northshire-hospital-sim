...
def generate_patients(n_patients, seed):
    np.random.seed(seed)
    ...
    def sample_chronic_conditions(age_val, imd):
        base = 0
        if age_val >= 65:
            base += 1
        if imd <= 3:
    ...
    chronic_conditions_count = np.array([
        sample_chronic_conditions(a, d)
        for a, d in zip(age, imd_decile)
    ])

    # 5. Dates
    def random_dob(age_val):
        years_delta = age_val
        # Random offset within +/- 6 months
        days_offset = np.random.randint(-183, 184)
        return (analysis_date - timedelta(days=365 * int(years_delta))) + timedelta(days=days_offset)
    ...
    registration_end_date = np.array([
        registration_end(start, active)
        for start, active in zip(registration_start_date, is_active)
    ])
